...
namespace HPY
{
	...
	public:
		...
		Window(const std::string& name = "Happy Engine", uint32_t width = 1920, uint32_t height = 1080);
		~Window();
		...
		glm::vec2 GetSize() const { return { m_WindowData.Width, m_WindowData.Height }; }
	...
	private:
		...
		struct WindowData
		{
			...
			WindowData(const std::string& name = "Happy Engine", uint32_t width = 1920, uint32_t height = 1080, bool vSync = true)
