#include "hpypch.h"
#include "Happy/Scene/SceneSerializer.h"
...
{
	template<>
	struct convert<HPY::TextureUniform>
	{
		static Node encode(const HPY::TextureUniform texUniform)
		{
			Node node;
		...
		static bool decode(const Node& node, HPY::TextureUniform& texUniform)
		{
	...
	bool SceneSerializer::Deserialize(const std::string& filePath)
	{
		...
		if (entities)
		{
			for (auto entity : entities)
			{
				...
				LOG_CORE_TRACE("Deserialized Entity with ID: {0}, Name: {1}", uuid, tag);
