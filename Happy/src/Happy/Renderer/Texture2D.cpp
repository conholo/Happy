...
namespace HPY
{
	...
	void Texture2D::SetData(const void* data, uint32_t size)
	{
		...
		{
			LOG_ERROR("Data must fill entire texture!");
		}
		else
		{
	...
	void TextureLibrary::Add(const Ref<Texture2D>& texture)
	{
		if (s_TextureLibrary.find(texture->GetName()) == s_TextureLibrary.end())
		{
			...
			LOG_CORE_TRACE("Added {} Texture to the Texture Library.", texture->GetName());
		}
		else
		{
			LOG_CORE_WARN("Texture already contained in Shader Library.  Attempted to add {} Texture to Library.", texture->GetName());
		}
	}
	...
	const Ref<Texture2D>& TextureLibrary::Get(const std::string& name)
	{
		...
		{
			LOG_CORE_ERROR("No texture with name \"{}\" found in Texture Library.", name);
			return nullptr;
		}
