#include "hpypch.h"
#include "Happy/Renderer/SceneRenderer.h"
...
#include "Happy/UI/PropertyDrawer.h"
...
namespace HPY
{
	...
	void SceneRenderer::InitializeSkybox()
	{
		...
		ShaderLibrary::Load("../Happy/resources/shaders/Skybox.shader");
		ShaderLibrary::Load("../Happy/resources/shaders/Preetham.shader");
	...
	{
		ShaderLibrary::Load("../Happy/resources/shaders/EditorGrid.shader");
	...
	void SceneRenderer::InitializeBloomPass()
	{
		...
		s_BloomProperties->BloomDirtTexture = TextureLibrary::Load(fileTexSpec, "../Happy/resources/textures/dirt-mask.png");
	...
	Ref<Framebuffer> SceneRenderer::GetMainColorBuffer()
	{
		...
		{
			LOG_CORE_ERROR("Main Color Buffer not found.  Geometry Pass (main color attachment) or Geometry Pass FBO is nullptr.");
			return nullptr;
		}
	...
	Ref<Framebuffer> SceneRenderer::GetDepthBuffer()
	{
		...
		{
			LOG_CORE_ERROR("Depth Buffer not found.  Shadow Pass or FBO is nullptr.");
			return nullptr;
		}
	...
	void SceneRenderer::ValidateResize(glm::vec2 viewportSize)
	{
		...
		{
			LOG_CORE_ERROR("Unable to resize framebuffer/viewport.  Geometry Pass (main color attachment) or Geometry Pass FBO is nullptr.");
			return;
		}

		if (s_SceneCompositePass == nullptr || s_SceneCompositePass->GetRenderPassSpecification().TargetFramebuffer == nullptr)
		{
			LOG_CORE_ERROR("Unable to resize framebuffer/viewport.  Composite Pass (main color attachment) or Composite Pass FBO is nullptr.");
			return;
		}
