...
namespace HPY
{
	...
	static ShaderDataType ShaderDataTypeFromGLenum(GLenum value)
	{
		switch (value)
		{
			...
			default: 
			{
				LOG_CORE_ERROR("Invalid GLenum.  No matching ShaderDataType found for {}.", value);
	...
	void Shader::DumpShaderData()
	{
		...
		LOG_CORE_INFO("-------------------------------------------------");
		LOG_CORE_INFO("Shader data for: {}", m_Name);
		LOG_CORE_INFO("-------------------------------------------------\n");
		LOG_CORE_INFO("Active uniform count: {}", m_ActiveTotalUniformCount);
		LOG_CORE_INFO("Base block uniforms: {}", m_DefaultBlockUniformCount);
		LOG_CORE_INFO("Named blocks: {}", m_Blocks.size());
		LOG_CORE_INFO("Named block uniforms: {}", m_NamedBlockUniformCount);
		LOG_CORE_INFO("\n");
		LOG_CORE_INFO("-------------------------------------------------");
		LOG_CORE_INFO("Base Block uniforms:");
		LOG_CORE_INFO("-------------------------------------------------");
		for (auto [name, uniform] : m_BaseBlockUniforms)
		{
			LOG_CORE_INFO("-------------------------------------------------");
			LOG_CORE_INFO("Uniform Name: {}", name);
			LOG_CORE_INFO("		Size: {}", uniform.GetSize());
			LOG_CORE_INFO("		Type: {}", ShaderDataTypeToString[uniform.GetType()]);
			LOG_CORE_INFO("		BufferOffset: {}", uniform.GetBufferOffset());
			LOG_CORE_INFO("		Location: {}", uniform.GetLocation());
			LOG_CORE_INFO("-------------------------------------------------");
		}

		LOG_CORE_INFO("\n");
		LOG_CORE_INFO("-------------------------------------------------");
		LOG_CORE_INFO("Named Blocks:");
		LOG_CORE_INFO("-------------------------------------------------");
		for (auto [blockName, block] : m_Blocks)
		{
			LOG_CORE_INFO("Block Name: {}", blockName);
			LOG_CORE_INFO("Block Binding: {}", block.GetBinding());
			LOG_CORE_INFO("Block Size: {}", block.GetBlockSize());
			LOG_CORE_INFO("Member Count: {}", block.GetMemberCount());

			for (auto uniform : block.GetUniforms())
			{
				LOG_CORE_INFO("-------------------------------------------------");
				LOG_CORE_INFO("		Uniform Name: {}", uniform.GetName());
				LOG_CORE_INFO("			Size: {}", uniform.GetSize());
				LOG_CORE_INFO("			Type: {}", ShaderDataTypeToString[uniform.GetType()]);
				LOG_CORE_INFO("			BlockOffset: {}", uniform.GetBlockOffset());
				LOG_CORE_INFO("-------------------------------------------------");
			}
	...
	void* Shader::GetUniformData(ShaderDataType type, GLint location)
	{
		switch (type)
		{
			case HPY::ShaderDataType::None:
			{
				LOG_CORE_ERROR("Unknown shader data type.");
				return nullptr;
			}
			case HPY::ShaderDataType::Float:
			case HPY::ShaderDataType::Float2:
			case HPY::ShaderDataType::Float3:
			case HPY::ShaderDataType::Float4:
			{
			...
			case HPY::ShaderDataType::Sampler2D:
			{
			...
			case HPY::ShaderDataType::Mat3:
			case HPY::ShaderDataType::Mat4:
			{
				LOG_CORE_INFO("We're currently unable to retrieve data from uniforms of type Mat3/Mat4.");
				return nullptr;
			}
		}
	...
	void Shader::DispatchCompute(uint32_t groupX, uint32_t groupY, uint32_t groupZ)
	{
		if (!m_IsCompute)
		{
			LOG_ERROR("{} is not of type Compute Shader.  Cannot dispatch.", m_NamedBlockUniformCount);
			return;
		}
	...
	void Shader::Reflect()
	{
		...
		if (activeCount == 0)
		{
			LOG_CORE_WARN("No active uniforms were found when reflecting {}.", m_Name);
			return;
		}
		...
		for (uint32_t i = 0; i < blockIndices.size(); i++)
		{
			...
			// Otherwise, query OpenGL for data about the block and construct a new Happy Block object to store the data.
			GLint binding;
			GLint blockSize;
			GLint activeCount;
	...
	void ShaderLibrary::Add(const Ref<Shader>& shader)
	{
		if (s_ShaderLibrary.find(shader->GetName()) == s_ShaderLibrary.end())
		{
			...
			LOG_CORE_TRACE("Added {} Shader to the Shader Library.", shader->GetName());
		}
		else
		{
			LOG_CORE_WARN("Shader already contained in Shader Library.  Attempted to add {} Shader to Library.", shader->GetName());
		}
	}
	...
	const Ref<Shader>& ShaderLibrary::Get(const std::string& name)
	{
		...
		{
			LOG_CORE_ERROR("No shader with name \"{}\" found in Shader Library.", name);
			return nullptr;
		}
